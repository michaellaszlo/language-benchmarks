...
public class SortingArrayInt {
	...
	static void trickleDown(int data[], int pos, int n) {
		int left = 2*pos+1;
		if (left >= n) {
			return;
		}
	...
	static void testHeapsort(int data[]) {
	...
	static void mergesort(int data[], int buffer[], int left, int right) {
		if (left+1 == right) {
			return;
		}
	...
	static void testMergesort(int data[]) {
		double startTime = System.currentTimeMillis();
	...
	static void quicksort(int data[], int left, int right) {
		...
		int ai = left, bi = (left+right)/2, ci = right-1,
	...
	static void testQuicksort(int data[]) {
		double startTime = System.currentTimeMillis();
	...
	static void testNativeSortInteger(int data[]) {
		double startTime = System.currentTimeMillis();
		Arrays.sort(data);
		double elapsed = (System.currentTimeMillis() - startTime)/1000;
		System.out.println(String.format(
	...
	static void permuteInts(int data[]) {
	...
	static boolean isSortedInt(int data[]) {
		int n = data.length;
		for (int i = 1; i != n; ++i) {
			if (data[i-1] > data[i]) {
				return false;
	...
	static void displayInts(int data[]) {
	...
	public static void main(String args[]) {
		...
		double startTime = System.currentTimeMillis();
		int unsortedIntegers[] = new int[n];
		for (int i = 0; i < n; ++i) {
